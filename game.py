...
def waiting_screen():
    ...
    draw_text(layar, "GALACTIC WAR!", 70, WIDTH/2, HEIGHT/4)
...
while running:
    ...
    for hit in hits:
        ...
        if isinstance(hit, Rock):
            ...
            hazard.add(rock)
            player.score_val +=1
        # cek apakah peluru mengenai alien
            ...
            if player.score_val % 30 == 0:
                hp += 100 #setiap skor kelipatan 30 HP Boss bertambah 100
                boss = Boss(hp)
    ...
    # ketika level lebih dari 2 rock meluncur lebih cepat
    if level >= 2:
        rock.speedx=random.randrange(-5,1)
        rock.speedy=random.randrange(5,10)

    if level >= 3:
        alien.speedx=random.randrange(-5,1)
        alien.speedy=random.randrange(5,10)
